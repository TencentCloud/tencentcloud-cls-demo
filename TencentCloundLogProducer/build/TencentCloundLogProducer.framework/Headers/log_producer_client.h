 ...
 */
LOG_EXPORT void
log_producer_client_network_recover(log_producer_client *client);
 ...
 *
 * @example  log_producer_client_add_log(client, 4, "key_1", "value_1", "key_2",
 * "value_2")
 ...
 */
LOG_EXPORT log_producer_result
log_producer_client_add_log(log_producer_client *client, int32_t kv_count, ...);
 ...
 * @return ok if success, LOG_PRODUCER_DROP_ERROR if buffer is full,
 * LOG_PRODUCER_INVALID if client is destroyed, LOG_PRODUCER_PERSISTENT_ERROR is
 * save binlog failed.
 */
LOG_EXPORT log_producer_result log_producer_client_add_log_with_len(
    log_producer_client *client, int32_t pair_count, char **keys,
    size_t *key_lens, char **values, size_t *value_lens, int flush);

/**
 * @note same with log_producer_client_add_log_with_len but use int32_t as
 * length add log to producer, this may return LOG_PRODUCER_DROP_ERROR if buffer
 * is full. if you care about this log very much, retry when return
 * LOG_PRODUCER_DROP_ERROR.
 *
 * @param client
 * @param pair_count key value pair count
 ...
 * save binlog failed.
 */
LOG_EXPORT log_producer_result log_producer_client_add_log_with_len_int32(
    log_producer_client *client, int32_t pair_count, char **keys,
    int32_t *key_lens, char **values, int32_t *value_lens, int flush);
 ...
 */
LOG_EXPORT log_producer_result log_producer_client_add_log_raw(
    log_producer_client *client, char *logBuf, size_t logSize, int flush);
 ...
 * @return same as log_producer_client_add_log_with_len
 */
LOG_EXPORT log_producer_result log_producer_client_add_log_with_array(
    log_producer_client *client, uint32_t logTime, size_t logItemCount,
    const char *logItemsBuf, const uint32_t *logItemsSize, int flush);

/**
 * add raw log buffer to client, this function is used to send buffers which can
 * not send out when producer has destroyed
 * @param client
 * @param log_bytes
 ...
 * save binlog failed.
 */
LOG_EXPORT log_producer_result log_producer_client_add_raw_log_buffer(
    log_producer_client *client, size_t log_bytes, size_t compressed_bytes,
