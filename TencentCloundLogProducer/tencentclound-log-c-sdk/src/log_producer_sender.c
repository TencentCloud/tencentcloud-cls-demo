...
void *log_producer_send_fun(void *param)
{
    ...
    if (send_param->magic_num != LOG_PRODUCER_SEND_MAGIC_NUM)
    {
        ...
        {
            producer_manager->send_done_function(producer_manager->producer_config->topic, LOG_PRODUCER_INVALID, send_param->log_buf->raw_length, send_param->log_buf->length,
    ...
    do
    {
        if (producer_manager->shutdown)
        {
            ...
            {
                producer_manager->send_done_function(producer_manager->producer_config->topic, LOG_PRODUCER_SEND_EXIT_BUFFERED, send_param->log_buf->raw_length, send_param->log_buf->length,
        ...
        sds accessKey = NULL;
        sds topic = NULL;
        log_producer_config_get_baseinfo(config, &accessKeyId, &accessKey, &topic);
        post_log_result *rst = post_logs_from_lz4buf(config->endpoint, accessKeyId, accessKey, topic, send_buf, &option);
        sdsfree(accessKeyId);
        sdsfree(accessKey);
        sdsfree(topic);
...
int32_t log_producer_on_send_done(log_producer_send_param *send_param, post_log_result *result, send_error_info *error_info)
{
    ...
    if (producer_manager->send_done_function != NULL)
    {
        log_producer_result callback_result = send_result == LOG_SEND_OK ? LOG_PRODUCER_OK : (LOG_PRODUCER_SEND_NETWORK_ERROR + send_result - LOG_SEND_NETWORK_ERROR);
        producer_manager->send_done_function(producer_manager->producer_config->topic, callback_result, send_param->log_buf->raw_length, send_param->log_buf->length, result->requestID, result->errorMessage, send_param->log_buf->data, producer_manager->user_param);
    }
    ...
    else
    {
        ...
        if (producer_manager->send_done_function != NULL)
        {
            producer_manager->send_done_function(producer_manager->producer_config->topic,
