...
ProducerManager *ConstructorProducerManager(ProducerConfig *producerconf)
{
    ...
    int32_t base_queue_size = producerconf->maxBufferBytes / (producerconf->logBytesPerPackage + 1) + 10;
    ...
    if (producerconf->topic != NULL)
    {
        producermgr->pack_prefix = _get_pack_id(producerconf->topic, producermgr->source);
    }
    else
    {
        producermgr->pack_prefix = _get_pack_id("default_logstore", producermgr->source);
    }
