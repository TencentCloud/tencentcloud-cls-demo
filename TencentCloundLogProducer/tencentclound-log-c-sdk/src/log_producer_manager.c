...
void _try_flush_loggroup(log_producer_manager *producer_manager)
{
    ...
    if (producer_manager->builder != NULL && now_time - producer_manager->firstLogTime > producer_manager->producer_config->packageTimeoutInMS / 1000)
    {
        ...
        if (rst != 0)
        {
            ...
            {
                producer_manager->send_done_function(producer_manager->producer_config->topic, LOG_PRODUCER_DROP_ERROR, loggroup_size, 0,
        ...
        do
        {
            ...
            if (data != NULL)
            {
                ...
                if (lz4_buf == NULL)
                {
                    ...
                    {
                        producer_manager->send_done_function(producer_manager->producer_config->topic, LOG_PRODUCER_DROP_ERROR, builder->loggroup_size, 0,
