...
int32_t AfterProcess(log_producer_send_param *send_param, post_result *result, send_error_info *error_info);
...
int32_t AfterProcess(log_producer_send_param *send_param, post_result *result, send_error_info *error_info)
{
    ...
    case LOG_SEND_QUOTA_EXCEED:
        ...
        cls_warn_log("send quota error, config : %s, buffer len : %d, raw len : %d, code : %d, error msg : %s",
    ...
    if (LOG_SEND_OK != send_result && error_info->last_send_error == 0)
    {
        ...
        error_info->last_sleep_ms = BASE_NETWORK_ERROR_SLEEP_MS;
        error_info->first_error_time = time(NULL);
        cls_warn_log("send fail, the error is discard data, retry once, config : %s, buffer len : %d, raw len : %d, total buffer : %d,code : %d, error msg : %s",
                     send_param->producerconf->topic,
                     (int)send_param->log_buf->length,
                     (int)send_param->log_buf->raw_length,
    ...
    else
    {
        ...
        if (producermgr->callbackfunc != NULL)
        {
            producermgr->callbackfunc(producermgr->producerconf->topic,
