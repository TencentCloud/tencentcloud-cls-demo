...
extern lz4_log_buf *
serialize_to_proto_buf_with_malloc_lz4(log_group_builder *bder);
extern lz4_log_buf *
serialize_to_proto_buf_with_malloc_no_lz4(log_group_builder *bder);
...
extern void add_log_full(log_group_builder *bder, uint32_t logTime,
                         int32_t pair_count, char **keys, size_t *key_lens,
                         char **values, size_t *val_lens);
extern void add_log_full_int32(log_group_builder *bder, uint32_t logTime,
                               int32_t pair_count, char **keys,
                               int32_t *key_lens, char **values,
                               int32_t *val_lens);
extern void add_log_full_v2(log_group_builder *bder, uint32_t logTime,
                            size_t logItemCount, const char *logItemsBuf,
                            const uint32_t *logItemsSize);
extern void add_log_raw(log_group_builder *bder, const char *buffer,
                        size_t size);
...
extern void add_tag(log_group_builder *bder, const char *k, size_t k_len,
                    const char *v, size_t v_len);
extern void add_pack_id(log_group_builder *bder, const char *pack,
...
// if you want to use this functions, add `-DADD_LOG_KEY_VALUE_FUN=ON` for
// cmake. eg `cmake . -DADD_LOG_KEY_VALUE_FUN=ON`
#ifdef LOG_KEY_VALUE_FLAG
/**
 * call it when you want to add a new log
 * @note sequence must be : add_log_begin -> add_log_time/add_log_key_value.....
 * -> add_log_end
 * @param bder
 */
 ...
 * @param value_len
 */
extern void add_log_key_value(log_group_builder *bder, const char *key,
